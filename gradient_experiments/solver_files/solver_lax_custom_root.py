...
def original_solve(m: Model, d: Data) -> Data:
...
def make_objective_function(m: Model, d:Data):
...
def make_solver_g(m: Model, d: Data, objective_f):
    ...
    def solve_iterative(f, qacc_guess: jax.Array):
        ...
        qacc_star = ctx_iter.qacc
        # Return qacc_star + auxiliary outputs
        return ctx_iter.qacc, (ctx_iter.qfrc_constraint, ctx_iter.efc_force)

    # Define a function g(h) = J(qacc_star) h using jax.jvp
    def g(h):
        _, jvp = jax.jvp(objective_f, (qacc_star,), (h,))
        return jvp  # This computes J(qacc_star) h without forming J explicitly

    return solve_iterative, g
...
def implicit_solve(m: Model, d: Data) -> Data:
    #jax.debug.print("Implicit Solve")
    """
    ...
    def tangent_solver(g, y):
        ...
        # Compute the Jacobian-vector product implicitly
        def matvec(h):
            return g(h)  # g(h) is J(qacc_star) h

        J = matvec()
        J = jax.jacobian(g)(y)
        jax.debug.print("Jacobian: {}", J)
        solution =  jp.linalg.solve(J, y)
    ...
    f = make_objective_function(m, d)
    solve, g = make_solver_g(m, d, objective_f=f)

    qacc_star, (qfrc_constraint, efc_force) = jax.lax.custom_root(
        f=f,
        initial_guess=qacc_guess,
        solve=solve,
        tangent_solve=lambda g, y: tangent_solver(g, y),
        has_aux=True
    )
