...
def _solve_iterations(m: Model, d: Data) -> Data:
...
def negate_or_zero_float0(grad_leaf, primal_leaf):
  ...
  if jax.dtypes.result_type(grad_leaf) == jax.dtypes.float0:
    # Replace with zeros of the same shape as primal_leaf, cast to a floating dtype.
    # By default, you can choose a float dtype. We pick float32 or match JAX defaults.
    return jp.zeros_like(primal_leaf, dtype=jp.float32)
  else:
    return -grad_leaf
...
def solve(m: Model, d: Data) -> Data:
  """
  This is exactly the same 'solve' in terms of API:
    def solve(m: Model, d: Data) -> Data
  except we mark it with @custom_vjp so that we can define
  an Implicit-Function-Theorem-based backward pass.
  """
  return _solve_iterations(m, d)

# Forward pass: just run the solver
def solve_fwd(m: Model, d: Data):
  d_out = solve.__wrapped__(m, d)  # run the real code, store result
  # We’ll store (m, d_out) for the backward pass
  return d_out, (m, d_out)

# A helpful "residual" that must be zero at the solution (for the IFT).
# We’ll define it as the final "grad" = M qacc - qfrc_smooth - qfrc_constraint,
# which should be zero if the solver has found equilibrium.
def _residual_fn(m: Model, d: Data, qacc_guess: jp.array) -> jp.array:
  """
  Returns the 'residual' vector that is zero if constraints are satisfied.
  We'll replicate the post-update logic:
    grad = (M @ qacc_guess - d.qfrc_smooth) - qfrc_constraint
  for that guess of qacc.
  """
  # Temporarily store qacc_guess in d
  d_tmp = d.replace(qacc=qacc_guess)
  # Create context with no iteration, just compute constraint + gradient
  ctx = _Context.create(m, d_tmp, grad=False)
  ctx = _update_constraint(m, d_tmp, ctx)
  ctx = _update_gradient(m, d_tmp, ctx)
  return ctx.grad  # shape = (m.nv,)

def solve_for_w_gd(R_fn, x_star, x_bar, step_size=1e-2, tol=1e-7, maxiter=1000):
  """
  Solve (dR/dx)^T w = x_bar using a minimal iterative gradient-descent approach.
  This is the 'transpose' of the forward-mode Jacobian-vector product.
  Args:
    R_fn: function qacc -> residual, shape (nv,)
    x_star: the final solution (qacc^*) we linearize around
    x_bar: the right-hand side for the linear system
    step_size: gradient descent step size
    tol: stopping threshold
    maxiter: maximum number of iterations
  Returns:
    w_final: solution to (dR/dx)^T w = x_bar
  """
  # We'll use jax.linear_transpose to apply (dR/dx)^T to a vector.
  def apply_dR_dqacc_T(vec):
    R_fn_vjp = jax.linear_transpose(lambda x: R_fn(x), x_star)
    # R_fn_vjp returns a 1-tuple because R_fn => shape (nv,)
    (res_x,) = R_fn_vjp(vec)
    return res_x
  # Gradient descent steps:
  def cond(state):
    w_val, iter_count = state
    # Residual is (dR/dx)^T w - x_bar
    r = apply_dR_dqacc_T(w_val) - x_bar
    return (jax.numpy.linalg.norm(r) > tol) & (iter_count < maxiter)
  def body(state):
    w_val, iter_count = state
    r = apply_dR_dqacc_T(w_val) - x_bar
    w_next = w_val - step_size * r
    return (w_next, iter_count + 1)
  w0 = jax.numpy.zeros_like(x_star)
  w_final, _ = jax.lax.while_loop(cond, body, (w0, 0))
  return w_final

def solve_bwd_gradient_descent(res, d_out_bar):
  m, d_out = res
  x_star = d_out.qacc
  x_star_bar = d_out_bar.qacc
  # 1) Solve (dR/dqacc)^T w = x_star_bar
  def R_fn(q):
    return _residual_fn(m, d_out, q)
  w = solve_for_w_gd(R_fn, x_star, x_star_bar)
  # 2) IFT for partial wrt. 'd' => -(dR/dd)^T w
  def R_d_fn(d_in):
    return _residual_fn(m, d_in, x_star)
  _, vjp_d = jax.vjp(R_d_fn, d_out)
  # d_bar = jax.tree_map(lambda x: -x, vjp_d(w)[0])
  d_bar = jax.tree_map(negate_or_zero_float0, vjp_d(w)[0], d_out)
  # if needed, do partial wrt m, etc.
  m_bar = None
  return (m_bar, d_bar)

from jax.scipy.linalg import solve as lin_solve
from jax.lax.linalg import lu as jax_lu
def solve_indefinite(J, rhs):
    # J is [nv, nv], might be indefinite
    # do partial pivoting LU
    lu, pivots, _ = jax_lu(J)
    # then do forward/back substitution with the pivot
    return jax.scipy.linalg.lu_solve((lu, pivots), rhs)

def solve_for_w_factor(R_fn, x_star, x_bar):
  """
  Solve (dR/dx)^T w = x_bar by building and factorizing dR/dx, then
  applying a one-shot solve with a linear algebra routine.
  Args:
    R_fn: function qacc -> residual, shape (nv,)
    x_star: final solution (qacc^*)
    x_bar: the right-hand side for the linear system
  Returns:
    w_final: solution to (dR/dx)^T w = x_bar
  """
  # 1) Build J = dR/dx via jacobian
  def R_only(x):
    return R_fn(x)
  J = jax.jacobian(R_only)(x_star)  # shape (nv, nv)
  # 2) Solve J.T @ w = x_bar
...
def solve_bwd_factorization(res, d_out_bar) -> Tuple[Optional[Model], Data]:
  ...
  def R_fn(q):
    return _residual_fn(m, d_out, q)
  w = solve_for_w_factor(R_fn, x_star, x_star_bar)
  # 2) IFT for partial wrt. 'd'
  def R_d_fn(d_in):
    return _residual_fn(m, d_in, x_star)
  _, vjp_d = jax.vjp(R_d_fn, d_out)
  # d_bar = jax.tree_map(lambda x: -x, vjp_d(w)[0])
  d_bar = jax.tree_map(negate_or_zero_float0, vjp_d(w)[0], d_out)
  # partial wrt m if needed...
  m_bar = None
  return (m_bar, d_bar)
# Register the forward/backward pass with the custom VJP:
solve.defvjp(solve_fwd, solve_bwd_factorization)
